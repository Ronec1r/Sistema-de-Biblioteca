...
def cadastro(): #função designada a CADASTRO dos livros
  ...
  if cod in usuario:
    ...
    if tip=='alu':
      ...
      usuario[cod]= {'nome': name, 'codigo': cod,'tipo': tip, 'emp': [], 'lim': 3, 'res': 3, 'data':[], 'exemplar': []}
    elif tip=='tec':
      print('Sucesso no cadastro!!')
      usuario[cod]= {'nome': name, 'codigo': cod,'tipo': tip, 'emp': [], 'lim': 4, 'res': 3, 'data':[], 'exemplar': []}
    elif tip=='pro':
      print('Sucesso no cadastro!!')
      usuario[cod]= {'nome': name, 'codigo': cod,'tipo': tip, 'emp': [], 'data':[], 'exemplar': []}
    else:
      print('Insucesso no cadastro. Tipo de usuário inválido!!')#dicionario de erros 
        ...
        if cod_livro in livros: 
            ...
            while i<len(usuario[cod]['emp']): #usar um loop para verificar cada item da lista de emprestimos
                if cod_livro==usuario[cod]['emp'][i]:
                    ...
                    del(livros[cod_livro]['exemp_emp'][i])
...
usuario= {
    200: {'nome': 'Alexsandra Prata', 'codigo': 200, 'tipo': 'pro', 'emp': [], 'data':[], 'exemplar': []},
    123: {'nome': 'Ana Luzia', 'codigo': 123, 'tipo': 'alu', 'emp': [], 'lim': 3, 'res': 3, 'data':[], 'exemplar': []},
    456: {'nome': 'Telma Alves', 'codigo': 456, 'tipo': 'tec', 'emp': [], 'lim': 4, 'res': 3, 'data':[], 'exemplar': []},
    789: {'nome': 'Alan Lucas', 'codigo': 789, 'tipo': 'alu', 'emp': [], 'lim': 3, 'res': 3, 'data':[], 'exemplar': []},
    100: {'nome': 'Paulo Perreira', 'codigo': 100, 'tipo': 'pro', 'emp': [], 'data':[], 'exemplar': []}
}
